...
![Frames from an 11-minute film above five higher-level feature time courses (motion energy, faces on screen, scene cuts, colour warmth, and voice) that affectlens extracted from it](docs/images/features.png)

<sub>Frames above mid-level feature time courses: motion energy (MT/V5), faces on
screen (FFA/OFA), scene cuts (hippocampus), colour warmth (V4/VO), and voice
(temporal voice areas), from [*Elephants Dream*](https://orange.blender.org/)
...
spaces* (each a hypothesis about what information drives the response) and fitting
a regularized linear model that predicts the measured signal from those features,
evaluated by its accuracy on **held-out** data [[1](#references)]. A feature space
that predicts held-out responses is *evidence that the information it encodes is
represented*, not proof of mechanism; this is the discipline the field holds
itself to [[1](#references), [2](#references)].
...
those **align onto a shared time grid** to form the design matrix **X** (bins by
features). From there, two workflows: a cross-validated **baseline** that predicts
human ratings **Y**, and an **encode** step that relates the features to a
separately recorded signal **s(t)** with a lag search.
...
Extraction turns each clip into exactly this **design matrix**, every feature as a
time course on one shared grid, the input both workflows consume:

![feature matrix: all 24 features over the film as a z-scored heatmap grouped by tier](docs/images/feature_matrix.png)

<sub>All 24 base features (mean-aggregated; `scene_cut` at its within-bin max),
...
| Faces | `face_count`, `face_prominence` | number of faces and how large/close the nearest one is | FFA / OFA / STS [[13](#references)] | detector-backed (bundled YuNet), the one mid-level feature that runs a model |
...
| Voice-band energy | `voice_band_ratio` | fraction of sound energy in the 300-3400 Hz speech band | temporal voice areas / STG [[14](#references)] | `spectral_centroid` is energy location; this is a speech-band fraction (a speech-presence proxy) |
...
functionally dissociable populations [[4](#references), [5](#references), [6](#references)].
...
[[12](#references)]. And `voice_band_ratio` is a speech-band energy statistic, not
...
- **Embedder:** text to vector. Default is a deterministic hashed bag-of-words
...
- **One shared time base.** Ratings and most recorded signals are sampled slowly
  and irregularly. Every feature stream, each on its own fast clock, is aggregated
  (mean / std / max) into the rating/feature bins, so correlation and regression
  are apples-to-apples.
- **Cross-validated, interpretable baselines.** Ridge regression with grouped
  (leave-one-clip-out) folds is the honest "predict an unseen clip" test, a clear
  reference a fancier model has to beat. Weights are an importance ranking,
...
> and cannot represent its rise, dispersion, and undershoot, nor adapt to region-
> or subject-varying latency; a mis-set lag attenuates the predicted *r* and can
> misattribute variance across correlated feature spaces. The principled upgrades
> are canonical / estimated **HRF convolution**, a **multi-delay FIR** model (a
> separate weight per feature and delay, the encoding-model default
...
**It runs end-to-end.** The test suite generates programmatic clips and drives the
entire chain (video/audio decode, feature extraction, time-grid alignment, rating
baseline, signal encoding), plus unit checks on every mid-level feature: a 220 Hz
tone reads as tonal and low-flatness while noise reads high-flatness, a uniform
pan reads as high-coherence and scrambled motion as low, a red frame reads warm,
speech-band energy concentrates in the voice band, the face summariser counts
boxes and finds the largest, and silence and flat frames stay finite. A green run
means every stage works together.
...
- **Visual:** facial-motion dynamism (mouth, blinks → posterior STS), scene /
  place category (→ PPA / RSC), animacy occupancy (animate vs. object → ventral
  temporal).
- **Audio:** speech envelope / amplitude modulation (→ STG speech tracking),
  trained voice-activity detection (→ temporal voice areas; beyond the current
  speech-band proxy), tempo / beat / onset density (→ auditory + SMA / basal ganglia).
...
end-to-end (decode, features, alignment, baseline, encoding), plus unit tests for
each mid-level feature. See [Machinery validation](#results-machinery-validation)
...
13. Kanwisher, N., McDermott, J., & Chun, M. M. (1997). The fusiform face area: a module in human extrastriate cortex specialized for face perception. *J. Neurosci.*, 17(11), 4302–4311. Haxby, J. V., Hoffman, E. A., & Gobbini, M. I. (2000). The distributed human neural system for face perception. *Trends Cogn. Sci.*, 4(6), 223–233.
14. Belin, P., Zatorre, R. J., Lafaille, P., Ahad, P., & Pike, B. (2000). Voice-selective areas in human auditory cortex. *Nature*, 403, 309–312.
